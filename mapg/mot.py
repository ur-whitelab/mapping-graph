...
class MOT:
    ...
    def build(self):
        ...
        self._graph.node[root]['atom_number'] = self._remove_bond(qgraph, self._G, root, symmetry=self._symmetry)

    def _remove_bond(self, graph, mol, parent, recurse=True, symmetry=False, layer=0):
        #check for end
        ...
        for bond in graph.edges():
            #print('Calling remove bond at layer', layer, 'with', graph.nodes(), ' and removing ', bond)
            ...
            if recurse:
                last_layer = self._remove_bond(_G, mol, nbunch, recurse, symmetry, layer + 1)
        return last_layer
